...
document.addEventListener('DOMContentLoaded', async () =>  {
    ...
    const updatePieChart = async () => {
        try {
            const response = await fetch(`/combined-data/?deviceID=${deviceID}`);
            if (!response.ok) {
                throw new Error('Network response was not ok');
            }
        
            const data = await response.json();
            const { today_gps_data, utilization_hours } = data;
            let activeHours = 0, inactiveHours = 0, idleHours = 0;

            if (today_gps_data.length === 0) {
                inactiveHours = 24;
            } else {
                today_gps_data.forEach(entry => {
                    switch (entry.state) {
                        case "Active":
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    function updateChart() {
        $.ajax({
            url: '/combined-data/?deviceID=' + deviceID,
            success: function(data) {
                // Extracting last 5 data from combined data
                var lastData = data.last_data;
                var gpsData = lastData.gps_data.map(item => item.speed);
                var extData = lastData.ext_data.map(item => item.speed);
                var labels = lastData.gps_data.map(item => item.time);
  ...
  function fetchDataAndUpdate() {
      fetch('/combined-data/?deviceID=' + deviceID)
          .then(response => {
              if (!response.ok) {
                  throw new Error('Network response was not ok');
              }
          ...
          .then(data => updateBarChart(data.utilization_hours))
